document.addEventListener('DOMContentLoaded', function() {
    // Register a plugin to draw text in the center of the doughnut
    const centerTextPlugin = {
      ...
      afterDraw(chart, args, options) {
        ...
        // Calculate total remaining time from tasks with less than 100% progress
        const totalRemaining = tasks.reduce((sum, task) => {
          if(task.progress < 100) {
            return sum + task.timeRequired * (1 - task.progress / 100);
          }
          return sum;
        }, 0);
        const text = totalRemaining.toFixed(0) + ' min remaining';
    ...
    function updateCategorySettings() {
      ...
      Object.keys(groupColors).forEach(category => {
        ...
        // Delete button for category using trash icon.
    ...
    taskForm.addEventListener('submit', function(e) {
      ...
      // If Parent Group is empty, assign "Uncategorized"
      if (!parentGroup) {
